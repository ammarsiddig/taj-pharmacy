...
export default function PaymentSettingsTab() {
  ...
  return (
    <div className="app-card space-y-4 p-5">
      ...
      <h3 className="text-base font-bold text-ink-main">Payment Settings</h3>

      <div className="grid grid-cols-2 gap-3">
        <input className={inp} placeholder="Name" value={form.name || ''} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
        <input className={inp} placeholder="Arabic Name" value={form.name_ar || ''} onChange={e => setForm(f => ({ ...f, name_ar: e.target.value }))} />
        <select className={inp} value={form.method_type} onChange={e => setForm(f => ({ ...f, method_type: e.target.value as PaymentMethodData['method_type'] }))}>
          ...
          <option value="bank_transfer">Bank / Wallet</option>
      ...
      <div className="flex gap-3">
        <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={!!form.is_default} onChange={e => setForm(f => ({ ...f, is_default: e.target.checked }))} />Default</label>
      ...
      <div className="app-card overflow-hidden">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-ivory-border bg-surface-secondary">
            <th className="px-4 py-2.5 text-right font-medium text-ink-muted">Name</th>
            <th className="px-4 py-2.5 text-right font-medium text-ink-muted">Type</th>
          ...
          <tbody>
            {methods.map(m => (
              <tr key={m.id} className="border-b border-ivory-border bg-white">
                ...
                <td className="px-4 py-2.5">{m.is_active ? <Badge variant="success">Active</Badge> : <Badge variant="neutral">Disabled</Badge>}</td>
                <td className="px-4 py-2.5 flex gap-2">
                  <button className="text-xs text-primary-600 hover:underline" onClick={() => edit(m)}>Edit</button>
                  <button className="text-xs text-status-danger hover:underline" onClick={() => remove(m.id)}>Delete</button>
                </td>
              </tr>
            ))}
          </tbody>

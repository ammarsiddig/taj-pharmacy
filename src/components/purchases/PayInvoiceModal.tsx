...
interface Props {
  ...
  payInvoiceForm: SchedulePaymentData;
  accounts: AccountRow[];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-[380px] rounded-2xl bg-white p-5 shadow-[var(--shadow-float)] border border-ivory-border">
        ...
        <p className="mb-4 text-sm text-ink-muted">
          {t('purchases.payInvoiceMsg')} — <span className="font-semibold tabular-nums text-ink-main">{api.formatMoney(remaining)}</span>
        </p>
        <div className="space-y-3">
          <div>
            ...
            <input
              ...
              onChange={e => onAmountChange(Math.round(parseFloat(e.target.value || '0') * 100))}
              className="app-input w-full px-3 py-2 text-sm"
              placeholder={api.formatMoney(remaining).replace(' SDG', '')}
            />
          </div>
          <div>
            ...
            <select
              ...
              onChange={e => {
                ...
                onFormChange({ ...payInvoiceForm, account_id: acctId, payment_method: derived });
              }}
              className="app-input w-full px-3 py-2 text-sm"
            >
              {accounts.map(a => (
          ...
          <div>
            ...
            <input
              ...
              onChange={e => onFormChange({ ...payInvoiceForm, payment_date: e.target.value })}
              className="app-input w-full px-3 py-2 text-sm"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={e => onFormChange({ ...payInvoiceForm, notes: e.target.value })}
              className="app-input w-full px-3 py-2 text-sm"
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          <Button onClick={onPay} disabled={!payInvoiceForm.account_id}>

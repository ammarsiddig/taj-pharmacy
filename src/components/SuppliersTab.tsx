...
export default function SuppliersTab() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const data = await api.getSuppliersFull(search || undefined, isActive);
      setSuppliers(data);
    } catch (err) {
  ...
  const handleToggleActive = async (s: SupplierRow) => {
    try {
      await api.toggleSupplierActive(s.id);
      loadData();
    } catch (err) {
      showToast('danger', String(err));
    }
    ...
    <div className="flex flex-col gap-4">
      ...
      {loading ? (
        ...
        <div className="sales-form-table-wrap app-card overflow-hidden">
          <table className="sales-form-table w-full text-sm">
            <thead>
              <tr>
                <th className="px-4 py-2.5 text-right font-medium text-ink-muted">{t('suppliers.name')}</th>
                <th className="px-4 py-2.5 text-right font-medium text-ink-muted">{t('suppliers.phone')}</th>
                <th className="px-4 py-2.5 text-right font-medium text-ink-muted">{t('suppliers.totalInvoices')}</th>
                <th className="px-4 py-2.5 text-right font-medium text-ink-muted">{t('suppliers.totalPurchased')}</th>
                <th className="px-4 py-2.5 text-right font-medium text-ink-muted">{t('suppliers.totalPaid')}</th>
                <th className="px-4 py-2.5 text-right font-medium text-ink-muted">{t('suppliers.balanceDue')}</th>
                <th className="px-4 py-2.5 text-right font-medium text-ink-muted">{t('suppliers.lastPurchase')}</th>
            ...
            <tbody>
              {suppliers.map((s) => (
                ...
                >
                  <td className="px-4 py-2.5 font-medium text-ink-main">{s.name_ar || s.name}</td>
                  <td className="px-4 py-2.5 text-ink-muted tabular-nums">{s.phone || '—'}</td>
                  <td className="px-4 py-2.5 tabular-nums text-ink-main">{s.total_invoices}</td>
                  <td className="px-4 py-2.5 tabular-nums text-ink-main">{api.formatMoney(s.total_purchased)}</td>
                  <td className="px-4 py-2.5 tabular-nums text-primary-600">{api.formatMoney(s.total_paid)}</td>
                  <td className="px-4 py-2.5 tabular-nums">
                    {s.balance_due > 0 ? (
                      <span className="text-status-danger font-medium">{api.formatMoney(s.balance_due)}</span>
                    ) : (
                      <span className="text-ink-muted">0.00</span>
                    )}
                  </td>
                  <td className="px-4 py-2.5 text-ink-muted tabular-nums">{s.last_purchase_date?.slice(0, 10) || '—'}</td>
                  ...
                  <td className="px-4 py-2.5">
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        title={s.is_active ? t('suppliers.inactive') : t('suppliers.active')}
                      >
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                          {s.is_active ? (
      ...
      {suppliers.length > 0 && (
        <div className="text-xs text-ink-muted">
          {suppliers.length} {t('suppliers.title')}
        </div>
      )}

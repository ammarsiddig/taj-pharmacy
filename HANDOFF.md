...
- Sync 500 on a *second* tenant (seen in live `pms-api` logs 2026-07-05, IP 41.95.138.167): `invalid input syntax for type boolean: "<timestamp>"` — a column-misaligned boolean from a stuck/old desktop client payload (same class as the TASK-940 duplicate-alias bug, different table). Separate from the TASK-943 `is_active` fix. Identify the offending snapshot query/table and confirm the client is on current desktop code.

### E2E day-in-the-life review findings (2026-07-05, against installed v0.2.25)

> From `tests/e2e` full-scenario run (`npm run e2e:full`). 53/56 steps passed; cash/bank/customer/supplier/stock reconciliations all exact. The failures below are real bugs, each caught by the suite. Full detail in `tests/e2e/REPORT.md`.

- **BUG (installed v0.2.25): `void_sale` is broken** — voiding any sale throws `CHECK constraint failed: movement_type IN (…)`; the void-reversal stock movement uses a `movement_type` the constraint doesn't permit. Sales cannot be voided at all on the shipped build. **Verify whether the current branch's migrations/allowed movement_types fix this; if not, it's a money-path blocker.**
- **BUG (installed v0.2.25): Session History silently empty** — `get_session_history` SQL references `r.deleted_at` (returns table has no such column) → throws; `SessionHistoryPanel` swallows it in a `catch {}` so the panel always renders 0 sessions. The query is already fixed on `fix/task-942` (uses `COALESCE((SELECT SUM(r.total) …))`); confirm it ships. Separately, the panel's silent `catch {}` should surface errors, not hide them.
- **BUG: desktop→cloud sync fails after a busy workflow** — `sync_all_tables_now` returns `500 Internal Server Error: Batch sync failed` once a run's worth of products/batches/movements/sales exist. Needs cloud-side investigation (which snapshot/batch upsert 500s).
- **BUG (Owner PWA): Activity page renders blank** — `/v1/activity` returns 200 but the page shows no filter chips, no items, and not even the empty state (caught by `npm run e2e:pwa`).
- **Gap: no reverse command for customer/supplier payments** — `record_customer_payment` / `record_supplier_payment` have no delete/reverse counterpart, so payments can't be undone in-app (E2E teardown relies on a DB snapshot restore). Consider a void/reverse path.
- **Note: license context not refreshed after in-session login** — `LicenseProvider` fetches once at mount; logging in after mount leaves features locked until the 5-min refresh. A `refresh()` on auth-change would fix it.

### E2E exhaustive review findings (2026-07-06, against installed v0.2.27)

> From `tests/e2e` exhaustive run (`npm run e2e:exhaustive`) — every screen/feature plus edge & negative paths, expiry-focused. 65/67 steps passed; all 10 reconciliation checks exact (cash/bank/stock). Teardown is a snapshot restore (real data verified pristine after). Full detail in `tests/e2e/REPORT.md`.

- **FIXED in v0.2.27 (confirmed): `void_sale`** — voiding now succeeds (8 sales voided cleanly in teardown, stock/credit reversed). The v0.2.25 CHECK-constraint bug is gone.
- **FIXED in v0.2.27 (confirmed): Session History** — `get_session_history` returns rows with no SQL error; the panel populates.
- **Sync no longer 500s on this run** — `sync_all_tables_now` succeeded (the v0.2.25 "Batch sync failed" did not reproduce on v0.2.27 for this dataset). The second-tenant boolean-misalignment 500 above is a separate, still-open cloud issue.
- **BUG (v0.2.27): `create_sale` accepts quantity 0** — creates an empty sale (`pos_sale_create.rs:123` checks only stock availability, never `qty > 0`). Add a `quantity >= 1` guard. **→ FIXED v0.2.28** (raw-input `quantity<=0` guard in `create_sale`+`create_invoice_sale`+`create_return`; see 2026-07-06 worklog).
- **BUG (v0.2.27): `create_sale` accepts NEGATIVE quantity** — a `-3` line is stored (total clamps to 0, FEFO loop skips it, but the line persists). Data-integrity risk; same missing `qty > 0` guard covers both. **→ FIXED v0.2.28** (same guard).
- **BUG (v0.2.27): below-cost / below-min_sale_price sale slips through when there is NO discount** — the margin guard (`pos_sale_create.rs:152`) is gated on `disc_amount > 0`, so a low `unit_price` entered directly bypasses it. It also compares against **cost**, not `min_sale_price`. Move the floor check out of the discount branch and check the effective per-unit price against `min_sale_price` for every line. **→ FIXED v0.2.28** (margin guard always runs, enforces `min_sale_price` floor with cost fallback, both POS and invoice paths).
- **Confirmed WORKING (no action):** expired-only sale blocked (POS + invoice), FEFO skips expired & sells earliest-valid first, past-expiry purchase rejected, expiry-today boundary inclusive, dispose+write-off, expiry-report buckets, low-stock alerts, oversell/split-mismatch/credit-limit-boundary/over-return blocks, same-account & over-transfer & customer-overpayment blocks, same-location & over-qty transfer blocks, stocktake discrepancy adjustment, opening-stock gated by setup_mode, duplicate-barcode & empty-name validation, soft-delete preserves history, tax into totals+report, park/hold cart round-trip.
- **Not covered by automation (needs manual/supervised):** role-based permissions (cashier/manager — needs their credentials), auth lockout after N wrong passwords, plan-tier license gating, CSV/Excel import bad-row handling, receipt-customizer visual preview, report CSV exports (front-end xlsx downloads). Listed explicitly in the report's coverage checklist.
...
### 2026-07-08 — Claude Code (Opus) — 11 live-test fixes on صيدلية الاحسان (v0.2.33)

- **Status:** DONE. Fixed all 11 issues found in live testing. `cargo check` clean, `npx tsc --noEmit` clean, `npm run build` clean, i18n ar/en parity kept for every new key. Verified the money/logic root-causes against the live DB (`%APPDATA%\com.taj.pharmacy\pharmacy.db`, tenant `7f4df371…`, branch `57501fb2…`).

**Money & logic (verified against DB):**
1. **Number formatting — missing thousands separators (ROOT FIX).** `src/api/core.ts::formatMoney` was `(piasters/100).toFixed(2)` → no grouping. Now uses `toLocaleString('en-US', {min/maxFractionDigits:2})` → `3,005,700.00`. This one change fixes all 207 call-sites (Dashboard cards, POS bar, reports, account balances). Audited `.toFixed(`/`toLocaleString`/raw `/100` renders app-wide; remaining `/100` are editable `NumericInput` fields (correct), and the local `money()` helpers in Sales/Assets/Warehouse already grouped.
4. **Dashboard "no backup" while backups exist.** `settings_backup.rs::get_auto_backup_status` filtered `backup_type = 'auto'`; the pharmacy only had `'local'` (manual) backups → dashboard showed "never". Now returns the latest **completed** backup of any type (`ORDER BY COALESCE(completed_at, started_at) DESC`). Relabeled the BackupTab strip from "آخر نسخ احتياطي تلقائي" → "آخر نسخة احتياطية" to match.
5. **Reports → Sales (and all report tabs) empty despite sales existing.** Root cause: 6 report tabs captured `const BRANCH = api.getBranchId()` at **module-load time**; `App.tsx` eagerly imports `Reports` → the const froze to the fallback `'main-branch'` **before login**, so every report queried a branch with 0 rows (dashboard worked because it reads the branch at runtime). Fixed all 6 (`SalesTab`, `BalanceSheetTab`, `ExpiryTab`, `InventoryTab`, `ProfitLossTab`, `TaxTab`) to read `api.getBranchId()` at call time.
9. **Accounts deactivate confusing + no delete guard.** (a) `get_accounts_summary` filtered `is_active = 1` → deactivated accounts vanished; now returns inactive too (totals still active-only), UI greys them and the Power toggle reactivates. Toggle toast now says `accountDeactivated`/`accountActivated` (was always the generic "تم حفظ الحساب"). (b) Added guarded `delete_account` (registered in `lib.rs`, `deleteAccount` api wrapper, Trash button + confirm modal): blocks delete when balance ≠ 0, when any `account_transactions` exist, or when bound to a `pos_session` — clear Arabic errors; soft-deletes and removes auto-created payment methods. Balances can never be orphaned.
10. **Accounts + payment-methods model locked (critical).** Documented + enforced the intended model in the backend:
   - **accounts** = real money containers (cash drawer / bank-wallet), per branch, with balance + is_active + (cash) is_default.
   - **payment_methods** = POS tenders. `cash` → money lands in the **session** cash account (method carries no account); `bank_transfer` → money lands in the **linked bank account** (account is **required**, must be an **active** `bank` account); `credit` → customer receivable, no account.
   - Enforcement (`catalog.rs`): `resolve_method_account()` requires+validates a live bank account for `bank_transfer` and forces `account_id = NULL` for cash/credit; `clear_default_for_type()` guarantees at most one default per method_type. `pos.rs::resolve_bank_payment_info` now also rejects a method whose linked account is inactive/deleted (defense-in-depth). Frontend `PaymentSettingsTab` shows the account picker only for bank_transfer (required, filtered to active bank accounts), clears it otherwise, and the hard-coded English "Saved" toast is now i18n.

**UX / visual:**
2. **Desktop icon padding.** Regenerated `src-tauri/icons/*` from a tightly-cropped, white-on-`#1C5F6F` square built from `taj-logo.svg` (source saved as `src-tauri/app-icon.svg`; bbox computed through the SVG transforms, mark fills ~85% of the width). Verified render. NOTE: the source SVG bakes the small "PHARMACY" wordmark into the same paths so "TAJ + bird only" couldn't be cleanly isolated — but the reported "too much empty padding" is resolved (mark now fills the square).
3. **Stock-alert colors.** Dashboard `AlertItem` rendered both states as pale 10%-opacity pastels. Now solid strong red `#DC2626` (out) vs solid amber `#D97706` (low) with white text — unmistakable at a glance.
6. **Inventory report reworked.** Backend now separates never-stocked catalog items (no batch history) from genuinely-depleted ones: `out_of_stock_items` requires batch history; added KPIs `stocked_products`, `never_stocked_count`, `reorder_count`, `zero_cost_items`. UI: KPI row (stock value / in-stock / reorder / catalog), a zero-cost warning + catalog note, margin hidden when costs are incomplete, and paginated (20/section) "show all" lists. This removes the 636-catalog-item noise.
7. **Invoice page polish.** `PurchaseDetail` summary grid / items-table / notes were flat `rounded-sm border`; added `rounded-xl` + `shadow-[var(--shadow-soft)]` (light, token-based).
8. **Invoice print upgraded.** `PrintInvoice.tsx`: proper letterhead (logo + name + address + phone + **license**), ruled items table with a shaded header + logical column alignment, and an emphasized boxed totals block now including **paid / remaining** (wired `amountPaid` from PurchaseDetail). Fixed a `left-0` → `start-0` RTL violation; `printColorAdjust: exact` so shaded blocks print.
11. **Smart-click rows.** Made Products rows (→ edit) and Purchases invoice rows (→ detail) clickable with pointer + hover, `stopPropagation` on inline action buttons; Customers/Suppliers/Accounts rows were already clickable.

- **Files changed:** `src/api/core.ts`, `src/api/accounts.ts`, `src-tauri/src/commands/{settings_backup.rs, accounts.rs, catalog.rs, pos.rs, reports_inventory.rs}`, `src-tauri/src/lib.rs`, `src/pages/reports/{SalesTab,BalanceSheetTab,ExpiryTab,InventoryTab,ProfitLossTab,TaxTab}.tsx`, `src/pages/{Accounts,Products,Purchases,PurchaseDetail,Dashboard}.tsx`, `src/pages/settings/{BackupTab,PaymentSettingsTab}.tsx`, `src/components/ui/PrintInvoice.tsx`, `src/types/reports.ts`, `src/i18n/{ar,en}.json`, `src-tauri/icons/*` + `src-tauri/app-icon.svg`, version bump 0.2.32→0.2.33.
- **Notes / follow-ups:** #2 can't isolate "TAJ+bird" from the "PHARMACY" wordmark without a separated design source. #7 "sales invoice" page had no separate flat detail page (sale receipt is the thermal `ReceiptBody`, already styled) so only the purchase detail was elevated. Pre-existing ar/en key-parity gaps (33 ar-only / 24 en-only) predate this task and were left untouched.

### 2026-07-07 — Claude Code — Live full-shift E2E on صيدلية الاحسان + 3 real bugs found & fixed
- **Status:** DONE. Ran the full "real pharmacy shift" as a live WebdriverIO E2E against the **installed v0.2.31** app driving the throwaway صيدلية الاحسان tenant (`fcc537bf-…`) through real Tauri commands — onboarding→catalog→opening stock→USD→purchases→POS→AR/AP→inventory→money→returns/void→cloud sync. **44/46 PASS.** Full report: `tests/e2e/REPORT-fullshift-live.md`.
- **USD v0.2.31 fix CONFIRMED LIVE:** activation @500/$ leaves prices unchanged (H1 500.00) → purchase sets 650.00 → rate ↑2× → **1,300.00** (pre-fix bug = 1,000.00) → rate ↓ @400/$ → 520.00; opening-stock H4 scales 300→240. Reconciliation exact: both funded accounts internally consistent, **no negative balances**; cash 3,025,700.00 and bank 2,961,000.00 both reconstruct to the piaster; customer AR 1,250.00; supplier net owed 50,000.00; 745 rows synced to `pharmacy.taj.systems` (0 failed).
- **3 real bugs found in dependent/financial chains — all FIXED in source (`cargo check` clean), NOT in the v0.2.31 binary (need rebuild to re-verify live):**
  1. **General supplier payment FK crash** — `suppliers.rs::do_supplier_payment` bound `invoice_id.unwrap_or("")` (empty string) into a column with an FK to `supplier_invoices`, so any payment-off-balance (no invoice) failed `FOREIGN KEY constraint failed`. Fixed: bind `Option<&str>` → NULL when empty.
  2. **Stock-take shrinkage logs inverted sign** — `warehouse_stocktake.rs` used `qty_change = difference.abs()`, so a −1 count loss recorded `stock_movements.quantity_change = +1`. Batch qty was correct but `SUM(quantity_change)` drifted above real on-hand (H2 ledger 141 vs physical 139). Fixed: `qty_change = difference`.
  3. **Purchase payment double-credits `amount_paid`** — `purchases.rs::confirm_purchase_with_payment` set `amount_paid = pay_amount` **and** then called `do_supplier_payment` which adds it again, so a 50,000 partial on a 100,000 invoice became `amount_paid=100,000 / "paid"`. Fixed: confirm UPDATE now sets `amount_paid = 0` and lets `do_supplier_payment` apply the single authoritative increment.
- **Re-verified live:** rebuilt the release binary (`npm run tauri build`) with all 3 fixes and re-ran the entire shift against it (`TAJ_APP_EXE` → `target/release/app.exe`, same profile, DB reset to clean snapshot) → **46/46 PASS** and exact reconciliation: supplier general payment now records (owed 30,000.00), stock-take adjust movement now `−1` (H2 ledger 139 = physical 139), invoice `amount_paid` correct (PUR-00002 partial = 50,000.00/"partial", not 100,000.00/"paid"). Accounts consistent, no negatives.
- **Recommend for owner:** bump v0.2.31 → **v0.2.32**, rebuild + reinstall from the produced NSIS installer (installed `Program Files\TAJ Pharmacy\app.exe` is still the old buggy binary), and add one regression test per bug. Build's final updater-signing error is expected without `TAURI_SIGNING_PRIVATE_KEY` and doesn't affect the binary/installer.
- **Files:** `src-tauri/src/commands/{suppliers.rs, warehouse_stocktake.rs, purchases.rs}`, `tests/e2e/desktop/specs/{fullshift-live,fullshift-sync}.e2e.js`, `tests/e2e/config/{wdio.fullshift,wdio.fullshift-sync}.js`, `tests/e2e/REPORT-fullshift-live.md`, `HANDOFF.md`.
...
- **Fix:** new `products::reanchor_sale_price` (re-derives the sale-price anchor only) is called at every non-owner-edit `sale_price` writer — `confirm_purchase`, `confirm_purchase_with_payment`, and opening-stock `find_or_create_product` (via `reanchor_product`). Bulk import already re-anchored; stocktake writes no price; create/update_product already anchored. `set_usd_rate`'s two SQL branches were extracted into shared `pub(crate)` helpers (`anchor_all_products_at_rate` / `reprice_all_products_to_rate`) so the command and the test drive identical code.
- **Test:** `src-tauri/tests/usd_anchor.rs` (integration test — drives the real helpers via a hidden `app_lib::test_support` re-export). Covers activation-doesn't-move-prices, the bug repro, the fix with rate up **and** down, the `MAX(1)` floor, and the opening-stock anchor gap. Integration test (not `#[cfg(test)]`) **because the in-crate unit-test build is broken** — see next.
- **Second bug found (pre-existing, NOT fixed):** `cargo test` does not compile — `commands/cloud_sync_tests.rs` is stale (missing `json!` import; calls now-private `run_background_scheduler_once`/`CloudSyncSchedulerConfig` after the 2026-05-15 scheduler refactor), and the `cfg(test)` builds of `cloud_sync_restore/snapshot` then throw ~70 serde_json inference errors. Recommend a dedicated fix pass.
- **Catalog:** built `tests/e2e/fixtures/dan-multi-activity-catalog.csv` — 629 enriched rows parsed from `PriceList 30 3 2026.pdf` (the Dan Multi Activity list) via `pdftotext -table`, with inferred Arabic category, normalized unit, generic name, and supplier/form/expiry in notes. ⚠️ prices are best-effort (source PDF column offset) — spot-check before import. The "Medical Plus ~400" list was not present in Downloads.
- **NOT executed live** (documented, not faked): fresh install + online activation of `PMS-0G1W-01DI-0XDP` (صيدلية الاحسان), the 629-row UI import, POS/session/reports clicks, and cloud sync — the E2E harness targets the **owner's real installed app + real DB/profile**, and there is no separate blank activatable instance; activating a different pharmacy or syncing a throwaway tenant would deactivate the owner and/or pollute **production**. Dependency chains (opening-stock movement_type vs purchases, FEFO `expiry ASC`, credit-limit `-1`/`0`/enforce, past-expiry rejection, balance/valuation/profit formulas) were **audited by code inspection — all PASS**. Full report + owner-runnable live procedure in `tests/e2e/REPORT-fullshift.md`.
- **Files:** `src-tauri/src/commands/{products.rs, purchases.rs, settings.rs, warehouse_opening_stock.rs}`, `src-tauri/src/lib.rs`, `src-tauri/tests/usd_anchor.rs`, `tests/e2e/fixtures/dan-multi-activity-catalog.csv`, `tests/e2e/REPORT-fullshift.md`, version trio + `Cargo.lock`. Version 0.2.30 → 0.2.31 (release not tagged — bump + worklog per task).

### 2026-07-07 — Claude Code — Brand logo rollout (desktop sidebar + Owner/Admin PWAs) + Release v0.2.30
- **Status:** DONE. `cargo check` clean (5 pre-existing warnings), desktop `tsc -b`+`vite build` clean, PWA `tsc -b`+`vite build` clean.
- **Continues** the brand relaunch (marketing + desktop `src-tauri/icons` were already done on `main`). Covers the three remaining surfaces.
- **Desktop in-app logo (`src/components/layout/Sidebar.tsx`):** the dark mark was barely visible on the `#1C5F6F` sidebar. Copied the white brand assets into `src/assets/` (`header-logo-white.svg` lockup, `taj-mark-white.svg`). Expanded rail now shows the white **TAJ/PHARMACY** lockup + `نظام إدارة الصيدلية` tagline (dropped the redundant `<h1>TAJ Pharmacy</h1>` — no TAJ twice); collapsed rail shows the white TAJ mark. Verified crisp on teal via a rendered preview.
- **Owner + Admin PWAs (`pms-cloud/web`, one shared build):** the Owner and Admin brand exports are byte-identical, so one icon set. Replaced `favicon.svg`/`.ico`, PWA `icon-192/512(+maskable)`, and `apple-touch-icon` from the new export; `index.html` now points at `favicon.svg` + `icons/apple-touch-icon.png` with a `?v=20260707` cache-buster; manifest already carries `purpose:"maskable"` on the maskable icons. Login + AdminLogin use the **color** header lockup on their light cards; the owner app's dark sidebar uses the **white** mark; refreshed the admin/install-banner marks. Removed the redundant `TAJ Pharmacy` footer on the owner login (the lockup already carries the name).
- **Deploy (PWA):** `pms-cloud/redeploy-web.ps1` (PMS_VPS_IP=taj.systems) → build + upload to `/opt/pms-cloud/web-dist` + `systemctl reload nginx`. **Verified live on `pharmacy.taj.systems`:** favicon.svg/.ico, header-logo-color.svg, apple-touch, icon-512-maskable, and the white owner mark all return 200 with the new bytes; a headless screenshot of the live login shows the color lockup rendering crisp with no duplicate footer.
- **Release v0.2.30 (desktop):** version trio + `Cargo.lock` → 0.2.30. The regenerated `src-tauri/icons` (already committed) + this sidebar change ship in the installer. Tagged `v0.2.30`, pushed → `release.yml` (tauri-action) publishes the GitHub release with `latest.json` = 0.2.30.
- **Files changed:** `src/components/layout/Sidebar.tsx`, `src/assets/header-logo-white.svg`, `src/assets/taj-mark-white.svg`, version trio + `Cargo.lock`; (PWA, separate commit) `pms-cloud/web/index.html`, `pms-cloud/web/public/*` (favicon/icons/logos), `pms-cloud/web/src/pages/Login.tsx`, `pms-cloud/web/src/pages/AdminLogin.tsx`.
...
- **Bug 1+2 — zero/negative sale quantity accepted (`pos_sale_create.rs`, `pos_invoice.rs`):** `create_sale`/`create_invoice_sale` only checked stock availability. A `qty<=0` line is silently collapsed to nothing by `resolve_fefo_items` (its batch loop breaks on `remaining<=0`), so the per-batch loop never sees it → an empty sale (`qty 0`) or a `-3` line with clamped total was created. **Fix:** added an early guard over the **raw input `items`** (before FEFO resolution) rejecting any `quantity<=0` with `الكمية غير صالحة للصنف {name} — يجب أن تكون أكبر من صفر`. Applied the same guard to the invoice path. Also closed the identical gap in **returns** (`pos_returns.rs`) — a negative return quantity would have *reduced* stock and refunded negatively; added `الكمية المرتجعة غير صالحة …`. **Void** (`pos_void.rs`) reads stored `sale_items`, which are now validated at creation, so no change needed.
- **Bug 3 — below-floor sale slipped through with no discount (`pos_sale_create.rs:~152`, `pos_invoice.rs:~156`):** the margin guard was gated on `disc_amount>0` and compared the discounted price to `unit_cost`, so a low `unit_price` entered with no discount bypassed it entirely and it never enforced `min_sale_price`. **Fix:** guard now **always runs**; loads `min_sale_price` alongside `unit_cost` into `item_costs`; computes each line's effective per-unit price after its share of any cart discount and rejects if it's below `floor = min_sale_price` (falling back to `unit_cost` when `min_sale_price` is 0/unset). Kept the existing below-cost message (now fires for the no-discount case too) and added a distinct `…أقل من الحد الأدنى للبيع…` message for the above-cost-but-below-min case. Identical rule applied to the invoice path.
- **Files changed:** `src-tauri/src/commands/pos_sale_create.rs`, `src-tauri/src/commands/pos_invoice.rs`, `src-tauri/src/commands/pos_returns.rs`, `package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`, `src-tauri/Cargo.lock`, `tests/e2e/desktop/specs/exhaustive.e2e.js` (updated the credit-over-limit step to use a valid ≥min price so it still exercises the credit-limit path rather than tripping the new margin floor).
- **Acceptance test result:** rebuilt the release exe (`tauri build`) and re-ran `npm run e2e:exhaustive` with `TAJ_APP_EXE` pointed at the fresh `target/release/app.exe`. Final run: **64/64 steps PASS, 0 failures, reconciliation 10/10 exact (cash 17,020.30 == 17,020.30, bank 0, stock), 0 bugs.** The 3 previously-failing steps now pass: qty 0 → `الكمية غير صالحة…`; qty −3 → same; below-cost `unit_price 50` (no discount) → `سيُباع بـ 5000 ج.س وهو أقل من سعر التكلفة…`. Normal sale still works; below-cost-via-discount still blocked; credit-over-limit now blocked with `تجاوز حد الائتمان المسموح به…`. DB restored to pre-run snapshot — verified pristine (cash 1,527,000, bank 0, zero E2E residue).
- **Notes:** the E2E harness normally targets the *installed* app; to verify pre-install I set `TAJ_APP_EXE` to the rebuilt binary (same `com.taj.pharmacy` app-data dir, so backup/restore + DB reconciliation apply unchanged). Plain `cargo build --release` is **not** enough to test through the UI — it embeds the dev URL (`localhost:5173`); a real `tauri build` is required to embed assets. The new NSIS installer `TAJ Pharmacy_0.2.28_x64-setup.exe` is in `src-tauri/target/release/bundle/nsis/` for the owner to install; updater signing still needs `TAURI_SIGNING_PRIVATE_KEY` (separate concern).

### 2026-07-05 — Claude Code — TASK-943 (finalize, v0.2.27) — void_sale CHECK fix + sync-500 fix + license refresh; finalize Copilot's in-tree POS/history/activity work
- **Status:** DONE. `cargo check` clean, `tsc`+`vite build` clean, PWA `npm run build` clean.
- **Continues** the prior Copilot CLI TASK-943 entry below (its POS-search/session-history/activity changes were left uncommitted). This entry adds the parts it did not cover and commits the whole set as clean, separated commits.
- **Bug 1 — void_sale CHECK constraint (CRITICAL, `src-tauri/src/db/migrations.rs`):** voiding any sale threw `CHECK constraint failed`. `pos_void.rs` writes `stock_movements.movement_type='void_sale'` and `account_transactions.transaction_type='sale_void'`, neither of which was in the CHECK lists. Added a TASK-943 migration extending both CHECKs via the documented rename-recreate-copy pattern (same as TASK-926 opening_stock) — additive, idempotent (guarded on the CHECK text), FK-safe (nothing references either table). Verified on a temp copy of the live DB: rows preserved, `void_sale`/`sale_void` inserts now accepted, guards don't re-trigger.
- **Bug 2 (Session History) — surface errors (`SessionHistoryPanel.tsx`):** in addition to Copilot's date-range + `r.deleted_at` fixes, replaced the silent `catch {}` with an error state + on-screen banner so a broken query is visible instead of an empty panel.
- **Bug 3 (intermittent sync 500, `pms-cloud/src/routes/sync.js`):** root cause was the soft-delete `UPDATE … SET is_active = false` — a boolean literal. `is_active` is `boolean` on some snapshot tables and `integer` on others, so any batch carrying `deletedIds` for an integer-`is_active` table (pos_sales, expenses, stock_movements, customer_payments …) threw `column is_active is of type integer but expression is of type boolean` and 500'd the whole batch (~50% of runs — only when a deletion, e.g. a void, was in the batch). Bound it as an untyped parameter (value 0) so Postgres coerces to whichever type the column is. Verified from live `pms-api` logs; deployed to the VPS (rebuilt `pms-api`); **5/5 consecutive syncs with deletions on integer+boolean tables returned 200**.
- **Bug 4 (license refresh, `src/hooks/useLicense.tsx`):** `LicenseProvider` now `useAuth()`s and re-fetches the license on auth-identity change (`isAuthenticated`/`tenant_id`/`user.id`) so features unlock immediately after an in-session login instead of waiting up to 5 min.
- **Activity (Owner PWA):** Copilot's `dashboard.js` `/v1/activity` reshape was already deployed to the VPS but never committed (found as working-tree drift); committed here. Re-verified the live query returns the expected shape (`event_type`,`entity_type`,`summary`,`occurred_at`,`received_at`) against real rows.
- **Versions:** this branch = **v0.2.27** (TASK-942 shipped 0.2.25 on it). Open PR #11 (payment methods) holds **0.2.26**. Recommended release order: **0.2.25 → 0.2.26 (PR #11) → 0.2.27 (this)**; because the void + sync-500 fixes are critical, if this must ship first, swap to 0.2.26 and rebase PR #11 to 0.2.27.
- **Backlog:** a *second*, unrelated sync 500 (`invalid input syntax for type boolean: "<timestamp>"`) is hitting **another tenant** (a stuck old-client payload with a column-misaligned boolean) — separate root cause, out of this task's scope; see §4.
- **Acceptance test:** `npm run e2e:full` (see run below / REPORT.md), esp. void now passing.
- **Status:** DONE
- **Files changed:** `src/pages/pos/SessionHistoryPanel.tsx`, `src-tauri/src/commands/pos.rs`, `src/pages/POS.tsx`, `src/pages/Sales.tsx`, `src/pages/pos/SearchResults.tsx`, `src/utils/posSearch.ts`, `src-tauri/src/commands/pos_invoice.rs`, `src/types/pos.ts`, `pms-cloud/src/routes/dashboard.js`, `pms-cloud/web/src/pages/Activity.tsx`, `package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`, `src-tauri/Cargo.lock`
- **Acceptance test result:** `cargo check` clean, `pnpm tsc --noEmit` clean, `pnpm build` clean, `cd pms-cloud/web && npm run build` clean. Live cloud deploy to `taj.systems` succeeded. Live `/v1/activity?limit=10` returned 4 recent activity rows after the fix. Live full snapshot replay to `https://pharmacy.taj.systems/v1/sync/batch` returned `success=True`, `totalUpserted=48`, `totalDeleted=0`. Live default Session History range SQL returned 2 closed sessions, including session `7f5c9309-3aaf-4340-8489-84b5858163ef` containing sale `SAL-00003` from `2026-07-05T11:26:18.325Z`.
- **Notes:** Issue 1 had **two** causes on the live DB: the panel defaulted `dateFrom/dateTo` to today only, and `get_session_history` also referenced `returns.deleted_at`, but the real `returns` table has no `deleted_at` column, so the query failed and the React panel swallowed the error as an empty list. Fixed by defaulting to the last 7 days and removing the invalid predicate. Issue 2/3 root cause was in `pms-cloud/src/routes/dashboard.js`: `/v1/activity` selected `snapshot_returns.total_amount` (column does not exist; real column is `total`), so `type=return` and `type=all` threw 500, and the route returned rows that did not match the PWA `ActivityItem` shape (`entity_type`, `entity_id`, `summary`, `received_at`), which made the page brittle. Fixed the route to return snapshot-derived business activity in the expected shape, deployed it, and updated the PWA Activity page to group/render by `received_at || occurred_at`. Confirmed separately that the desktop outbox/events path is healthy **but only carries owner-dashboard events** right now: live `sync_events` contains `snapshot|owner_dashboard|235` and `refresh_request|owner_dashboard|26`, and live `activity_log` contains `snapshot|owner_dashboard|76` and `refresh_request|owner_dashboard|15` — no sale/product/etc. business events are being published through `/v1/events`, so the Owner Activity page must rely on snapshot-derived `/v1/activity` data. Issue 4 root cause was UI-side batch expansion plus invoice-sales requiring explicit `batch_id`, compounded by search batch ordering that did not honor FEFO. Fixed the POS and invoice sale searches to show one row per product with aggregated available quantity, send `batch_id: undefined` for aggregated sale rows so Rust resolves FEFO server-side, and aligned `search_products_pos` / substitutes ordering with `resolve_fefo_items` (`ORDER BY expiry_date ASC NULLS LAST, created_at ASC`). Live stock verification on `Panadol Extra` (`5e05d8ee-78a2-4fcd-acf1-c4565efaef4b`) showed 2 active batches totaling 15 units; the corrected FEFO order now picks dated batch `e6b3bad3-e03e-4f2e-a77a-81a6ddafdb8d` (`expiry_date=2027-06-30`) before null-expiry batch `967a45f3-efae-433b-a6a4-41742b98b446`. Historical live sales showed the old bug clearly: `SAL-00002` and `SAL-00003` both consumed the null-expiry batch first.

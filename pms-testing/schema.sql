-- PMS Pharmacy (TAJ) - PostgreSQL Test Schema
-- Regenerated 2026-05-19 from src-tauri/src/db/migrations.rs
-- Mirrors the final desktop SQLite state in PostgreSQL syntax.
-- No seed data. Apply with: psql -f schema.sql
...
CREATE TABLE IF NOT EXISTS tenants (
    ...
    pending_update_checked_at TEXT,
...
CREATE TABLE IF NOT EXISTS accounts (
    ...
    is_active        INTEGER NOT NULL DEFAULT 1,
    bank_provider    TEXT,
    internal_fee     INTEGER NOT NULL DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS idx_account_transactions_tenant
    ON account_transactions(tenant_id);
...
CREATE INDEX IF NOT EXISTS idx_customer_payments_tenant
    ON customer_payments(tenant_id);
...
CREATE INDEX IF NOT EXISTS idx_pos_sessions_branch
    ON pos_sessions(branch_id, status) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_sales_tenant
    ON sales(tenant_id, sale_date) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_sales_branch
    ON sales(branch_id, sale_date) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_notifications_tenant
    ON notifications(tenant_id, is_read) WHERE deleted_at IS NULL;
